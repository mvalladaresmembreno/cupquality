document.onreadystatechange = function () {
    if (document.readyState === 'complete') {
        ...
        const OT = document.getElementById("ordenTrabajo");
        OT.addEventListener("keyup", () => {
            try {
                let value = event.target.value;
                const request = new XMLHttpRequest(); 
                request.open('GET', '/getajax/getOT/'+ value);
                request.onload = () => {
                    const data = JSON.parse(request.responseText);
                    console.log(data.exists);
                    if (data.exists == true)
                    {
                        OT.classList.remove("is-valid");
                        OT.classList.add("is-invalid");
                    }
                    else
                    {
                        OT.classList.remove("is-invalid");
                        OT.classList.add("is-valid");
                    }
                };
                request.send();
            }
...
const saveMuestra = (event, nuevo)=>{
    ...
    const request = new XMLHttpRequest();
    request.open('POST', '/lab/addMuestra');
    request.onload = ()=>{
        const data=JSON.parse(request.responseText);
        if (data.status==200){
            if(nuevo==true){
                ...
                window.location.replace("/lab/verMuestras");
            }
        }else{
            alert(data.mensaje);
            return false;
...
const editMuestra = (event, idMuestra)=>{
    ...
    const request = new XMLHttpRequest();
    request.open('POST', '/lab/editMuestra/'+idMuestra.toString());
    request.onload = ()=>{
        const data=JSON.parse(request.responseText);
        if (data.status==200){
                alert(data.mensaje);
                window.location.replace("/lab/editMuestra/"+idMuestra.toString());
        }else{
            alert(data.mensaje);
            return false;
        }
