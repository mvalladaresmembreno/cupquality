...
class Muestra(TimeStamped):
    ...
    ordenTrabajo=models.IntegerField(verbose_name="Orden de Trabajo", null=True, blank=False, unique=True)
    fechaEntrega=models.DateField(verbose_name="Fecha de Recepción",null=False)
    codigoPais=models.CharField(max_length=4, verbose_name="Código de País", null=False, blank=False, default="017")
    exportador=models.CharField(max_length=64 ,verbose_name="Exportador", null=False, blank=False, default="0000")
    lote=models.IntegerField(verbose_name="Lote", null=False, blank=False, default=0000)
    peso=models.CharField(max_length=4, choices=PESOS, default="500")
    ...
    fkOrg=models.ForeignKey(Organizacion, blank=True, null=True, related_name="LOrgMuestra", verbose_name="Organización", on_delete=models.CASCADE)
    sedeRecepcion=models.ForeignKey(Sede, blank=True, null=True, related_name="LABSedeMuestra", verbose_name="Sede de Recepción", on_delete=models.CASCADE)
    sedeAnalisis=models.ForeignKey(Sede, blank=True, null=True, related_name="LABSedeAnalisis", verbose_name="Sede de Analisis", on_delete=models.CASCADE)
...
class Fisico(TimeStamped):
    fkCatador=models.ForeignKey(Colaborador, blank=False, related_name="LCatadorFisico", verbose_name="CatadorFisico", on_delete=models.CASCADE)
    fkMuestra= models.ForeignKey(Muestra, blank=False, related_name="LFsico", verbose_name="Muestra", on_delete=models.CASCADE)
...
class Sensorial(TimeStamped):
    fkCatador=models.ForeignKey(Colaborador, blank=False, related_name="LCatadorSensorial", verbose_name="CatadorSensorial", on_delete=models.CASCADE)
    fkMuestra= models.ForeignKey(Muestra, blank=False, related_name="LSensorial", verbose_name="Muestra", on_delete=models.CASCADE)
    ...
    pAroma=models.FloatField(null=False, blank=True, default=0.0)
    ...
    def total(self):
        if self.iCuerpo == 3:
            self.castigo=(4*(5-self.cTLimpia))
            self.pFinal=(self.fragancia+self.balance+self.pSabor+self.pTLimpia+self.remanente+self.acidez+self.cuerpo+self.pUniformidad +self.pCatador) - (4*(5-self.cTLimpia))
        elif self.iCuerpo == 1 or self.iCuerpo == 2:
            self.castigo=(2*(5-self.cTLimpia))
            self.pFinal=(self.fragancia+self.balance+self.pSabor+self.pTLimpia+self.remanente+self.acidez+self.cuerpo+self.pUniformidad +self.pCatador) - (2*(5-self.cTLimpia))
        self.save()

    def getIntensidad(self):
        return self.get_intensidad_display()
    
    def getTueste(self):
        return self.get_nTueste_display()

class Analisis(TimeStamped):
    fkTecnico=models.ForeignKey(Colaborador, blank=False, related_name="LATecnico", verbose_name="Tecnico", on_delete=models.CASCADE)
    fkMuestra=models.ForeignKey(Muestra, blank=False, related_name="LAMuestra", verbose_name="Muestra", on_delete=models.CASCADE)
    fkFisico=models.ForeignKey(Fisico, blank=True, related_name="LAFisico", verbose_name="Fisico", on_delete=models.CASCADE)
    fkSensorial=models.ForeignKey(Sensorial, blank=True, related_name="LASensorial", verbose_name="Sensorial", on_delete=models.CASCADE)
    oAnalisis=models.IntegerField(choices=oAnalisis, default=1)
    fechaAnalisis=models.DateField(null=False, blank=True)
    PuntajeFinal=models.FloatField(null=False, blank=True, default=0.0)
    estado=models.IntegerField(choices=Estado.choices, default=2)

    def getEstado(self):
        return self.get_estado_display()
    def getObjetivo(self):
        return self.get_oAnalisis_display()

class Conciliacion(TimeStamped):
    fkAnalisis=models.ForeignKey(Analisis, blank=False, related_name="CAnalisis", verbose_name="Analisis", on_delete=models.CASCADE)
    ...
    fkCFisico=models.ForeignKey(Fisico, null=True, blank=True, related_name="CFisico", verbose_name="Fisico" , on_delete=models.CASCADE)
